...
def checkWin(B):
    ...
    for k in range(5, 2, -1):
        # uses also the variable p to control the column starting point in the table
        for p in range(3, 7):
...
# ---------------AI FUNCTIONS-------------------------------------
# these AI functions are first-wave AI
def fourBlocking(B):
    # this function takes in the gameboard
    ...
    for i in range(5, -1, -1):
        for p in range(0, 5, 1):  # using the variable p to control the column starting point
            ...
            if (B[i][p] == B[i][p+1] and B[i][p+1] == B[i][p+2] and B[i][p] != " " and B[i][p] == "X"):
                if (p == 0 or p == 4):  # taking care of edgecases
                    ...
                    # check if both the adjacent boxes are empty
                    if (B[i][p-1] == " " and B[i][p+3] == " "):
                        if (i == 5):  # taking care of the row edgecase
                            # uses the random variable to randomly block before or after
    ...
    if (found == False):  # if no horizontal block is needed,it checks for vertical ones
        for j in range(0, 7, 1):
            for k in range(5, 2, -1):  # using the variable k to control the row starting point
                # check if there's a three-in-a-row
                if (B[k][j] == B[k-1][j] and B[k-1][j] == B[k-2][j] and B[k][j] != " " and B[k][j] == "X"):
...
def ZeroesContinue(B, x):
    # this function takes in the gameboard and the pawn type
                    ...
                    if (i == 5):  # taking care of the row edgecase
                        ...
                        if (B[i][p] == B[i][p+1] and B[i][p] == x):
                            if (B[i][p-1] == " " and B[i][p+2] == " "):
                                found = True
                                # chooses randomly between the two possible places
                        ...
                        if (B[i][p] == B[i][p+1] and B[i][p] == x):
                            if (B[i][p-1] == " " and B[i][p+2] == " " and B[i+1][p-1] != " " and B[i+1][p+2] == " "):
                                ...
                                var = rand
                                break
                            # checks if the box before is empty
                            elif (B[i][p-1] == " " and B[i+1][p-1] != " "):
                                found = True
                                var = 1
                                break
                            # checks if the box after is empty
                            elif (B[i][p+2] == " " and B[i+1][p+2] != " "):
                                found = True
                                var = 2
                                break
                            else:
    ...
    if (found == False):
        for p in range(2, 7, 1):
            for k in range(5, 1, -1):  # again, using p and k as starting points
                ...
                if (B[k][p] == B[k-1][p-1] and B[k][p] == x):
                    ...
                    if (i == 5):  # taking care of the first row edgecase
                        if (j == 0):  # taking care of first coulmn edgecase
                            if (B[i-1][j] == " "):  # checks if the box above is empty
                                ...
                                x = j+1
                                break  # break as soon as it finds a build
                            # looks for a right diagonal build
                            ...
                            if (B[i-1][j] == " "):  # checks if the box above is empty
                                ...
                                x = j-1
                                break  # break as soon as it finds a build
                            # looks for a left diagonal build
                            elif (B[i][j-1] != " " and B[i-1][j-1] == " "):
        ...
        for i in range(5, -1, -1):
            for j in range(0, 7, 1):
                ...
                if (B[i][j] == y):
                    if (i == 5):  # taking care of the first row edgecase
                        if (j == 0):  # taking care of first coulmn edgecase
                            # checks if the box above is empty
                            if (B[i-1][j] == " " and var == 0):
                                found = True  # a vertical build can be done
                                x = j
                                break  # break as soon as it finds a build
                                ...
                                break  # break as soon as it finds a build
                            # looks for a right diagonal build
                            elif (B[i][j+1] != " " and B[i-1][j+1] == " " and var == 4):
                                found = True  # a right diagonalbuild can be done
                                x = j+1
                                break  # break as soon as it finds a build
                            else:
                            ...
                            if (B[i-1][j] == " " and var == 0):
                                ...
                                break  # break as soon as it finds a build
                            # looks for a left diagonal build
                            elif (B[i][j-1] != " " and B[i-1][j-1] == " " and var == 3):
                                found = True  # a left diagonal build can be done
                                x = j-1
                                break  # break as soon as it finds a build
                            else:
...
def main():
    ...
    print("------------------------------")
    undo_choice = int(
        input("Do you want to be able to undo your moves during the game?(1/2):"))
    while (undo_choice != 1 and undo_choice != 2):  # forces the user to enter a valid choice
        print("------------------------------")
        print("Your choices are:")
        print("1.Yes")
        print("2.No")
        print("------------------------------")
        undo_choice = int(input("Please enter a valid choice:"))

    # it starts the game itself
    while (True):  # creates an infinite loop for the game
        # saves the state of gamboard before the turns begin
        K = StoreBoardState(gameBoard)
        ...
        # finds the row index of the box that should be filled
        r0 = rowPlacement(gameBoard, x, turn)
        # modifies the gambeoard by filling the right spot
        gameBoard[r0][x] = "X"
        ...
        if (z != -1):  # check if the computer has to block a three-in-a row
            ...
            if (w != -1):  # check if there is XX_X type trap to be blocked
                ...
                # creates a random variable to build on an X or O
                build_choice = random.randint(0, 1)
                if (build_choice == 0):  # the program chooses to build on an X
                    # first,it tries to build on an X
                    buildX = ZeroBuilder(gameBoard, "X")
                    if (buildX != -1):
                        buildX = ColumnReassign(gameBoard, buildX, turn)
                        r1 = rowPlacement(gameBoard, buildX, turn)
                        gameBoard[r1][buildX] = "O"
                    else:  # then, it tries to build on an O
                        build = ZeroBuilder(gameBoard, "O")
                        if (build != -1):  # the computer can build zero structures
                            ...
                            # it reassigns it if it's full
                            rand = ColumnReassign(gameBoard, rand, turn)
                            # finds the row index of the box that should be filled
                            r1 = rowPlacement(gameBoard, rand, turn)
                            gameBoard[r1][rand] = "O"
                elif (build_choice == 1):  # it chooses to build on a O
                    build = ZeroBuilder(gameBoard, "O")
                    if (build != -1):  # the computer can build zero structures
                        build = ColumnReassign(gameBoard, build, turn)
                        r1 = rowPlacement(gameBoard, build, turn)
        ...
        if (undo_choice == 1):
            ...
            if (undo == 2):  # it undoes the move
                ...
                displayBoard(gameBoard)  # displays the gameboard on the screen
            while (undo != 1 and undo != 2):  # force the user to enter a valid choice
                print("------------------------------")
                print("Do you want to continue or to undo your action?")
                print("1.Continue")
                print("2.Undo")
                print("------------------------------")
                undo = int(input("Please enter a valid choice:"))
