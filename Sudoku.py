# Sudoku Game Program w/ file use
import os
...
class Sudoku:
        ...
        try:
            if (control == 1 or control == 2):
                ...
                while (choice[0] > 9 or choice[0] < 1):  # force a valid choice
                    choice[0] = int(
                        input("Enter a valid number between 1 and 9:"))
            else:
                choice[0] = 0
            ...
            if (control == 1):
                # it is full so it must be reassigned
                while (self.CheckFullSpace(choice[1], choice[2]) == True):
                    print("-------------------------------------------")
            ...
            return self.UserInput(self, control)
    ...
    def CheckValidPlacement(self, a, i, j):
        ...
        if (valid == True):
            for k in range(9):
                # checks if the value is found in the row
                if (self.allBoard[i][k] == a):
                    valid = False  # the values has been found
                    break
        ...
        if (valid == True):
            ...
            for c in range(3 * k, 3 * k + 3):
                for d in range(3 * p, 3 * p + 3):
                    # checks if the value is found in the box
                    if (self.allBoard[c][d] == a):
                        valid = False  # the values has been found
                        break
                if (valid == False):
                    break
    ...
    def placementReassignment(self, a, i, j, control):
        ...
        while (self.CheckValidPlacement(x[0], x[1], x[2]) == False):
            print(
                "There must be just one instance of each number in each column, row, and box")
            print(
                "---------------------------------------------------------------------------")
...
def main():
    ...
    while (True):
        if (flag == True):
            ...
            x = game.UserInput(1)
            # reassigns to a valid place
            x = game.placementReassignment(x[0], x[1], x[2], 1)
            ...
            if (choice2 == 2):
                ...
                if (game.CheckFullSpace(x[1], x[2]) == True):
                    ...
                    if (game.board[x[1]][x[2]] != " "):
                        y = game.UserInput(2)
                        y = game.placementReassignment(
                            y[0], x[1], x[2], 2)  # reassigns to a valid one
                        # assign the value to the user matrix and to the unified one
